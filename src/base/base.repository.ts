...
export class BaseRepository<T> {
    ...
    public async find(options: {
        ...
        const { offset, limit = 10, filter, sort, selectedFields, populates } = options ?? {};
    ...
    public async updateById(id: string, data: Partial<T>, options?: QueryOptions): Promise<T> {
    ...
    public async bulkWrite(data: Array<any>): Promise<void> {
        await this._model.bulkWrite(data);
    }

    public async aggregate(pipelines: Array<any>): Promise<Array<any>> {
        return await this._model.aggregate(pipelines).allowDiskUse(true).exec();
    }

    // public async aggregateCursor(pipelines: Array<any>, batchSize: number) {
    //     return this._model.aggregate(pipelines).allowDiskUse(true).cursor({ batchSize: batchSize });
    // }
